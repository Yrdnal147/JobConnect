...
# We'll just replace the whole file because it's easier and avoids regex mishaps with massive UI changes.
...
class _ConversationsPageState extends State<ConversationsPage> {
  ...
  Widget _buildFilterChips(MessagingState state) {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          _buildChip('Toutes', 'all', currentFilter == 'all'),
          const SizedBox(width: AppSpacing.sm),
          _buildChip('Non-lues', 'unread', currentFilter == 'unread'),
          const SizedBox(width: AppSpacing.sm),
          _buildChip('Archivées', 'archived', currentFilter == 'archived'),
        ],
      ),
    );
  }
...
with open(filepath, 'w', encoding='utf-8') as f:
    f.write(new_content)
print("Rewrote ConversationsPage")
